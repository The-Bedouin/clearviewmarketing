...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const scale = snapOnStack && scrollTop >= triggerStart ? targetScale : 1 - scaleProgress * (1 - targetScale);
      const rotation = snapOnStack && scrollTop >= triggerStart
        ? 0
        : (rotationAmount ? i * rotationAmount * scaleProgress : 0);
  ...
  const setupLenis = useCallback(() => {
    ...
    return lenis;
  }, [handleScroll, enableSmoothScroll]);

  useLayoutEffect(() => {
    const scroller = scrollerRef.current;
    if (!scroller) return;

    // Ensure proper overflow depending on scroll mode
    scroller.style.overflow = enableSmoothScroll ? 'hidden' : 'auto';
    ...
    cards.forEach((card, i) => {
      ...
      if (useStickyPin) {
        ...
        card.style.top = `${Math.round(stackPositionPx)}px`;
      } else {
        card.style.position = '';
        card.style.top = '';
      }
